...
//Tamaño de la pantalla - variable global (uniform)
...
function drawScene(gl, vao, programInfo, bufferInfo) {
  //Hacer que el tamaño del canvas sea
  twgl.resizeCanvasToDisplaySize(gl.canvas);

  gl.viewport(0, 0, gl.canvas.width, gl.canvas.height);

  //lo que se le manda al shader
  let uniforms = {
...
function generateData(sides, centerX, centerY, radius) {
  ...
  for (let s = 0; s < sides; s++) {
    ...
    // Generate the coordinates of the vertex -> se le suma el offset
    ...
    //Reutilizar el vertice con el color
